...
Hi, my name is Torey Lomenda. I am local to the Twin Cities, and currently am a VP of Solution Architecture & Delivery for Improving. Improving is a modern digital services sofware development, consulting, and training company with over 20 offices around the world providing onshore, nearshore, offshore options to our clients (a.k.a. "all-shore consulting services").

Today, I am going to take you on a magical journey OR "at the very least" a practical solution architect's journey on adding context to code to build software that works. I hope to keep this fun while sharing tips & techniques I consider beneficial.

One thing I hope you take away from the walkthrough and demo is building better software is always achievable and not a fantasy. Those who dare can embark on a journey "building software that WORKS, through a story you CONNECT with, using solution methods to GUIDE delivery".

The real world might be "messier" than today's journey, but the methods I highlight, with practice, lead to iteratively delivering value-add outcomes in your SDLC (software development lifecycle)
...
So I stumbled on this nice tutorial with all the tech buzzwords I was looking for. It was a good start - but with many tutorials, can be easily forgotten after going through it. 

However, adding color with a relatable story and design methods, along with a fully integrated playground "system" running on Docker it has become a powerful learning tool for ongoing experimentation.  
...
Domain Storytelling is a pictorial language involving actors, work objects, and the activities/interactions between them. This simple, yet powerful method aligns well with domain-driven design building blocks and is great at depicting TO-BE and AS-IS processes at various levels of granularity.
...
After my working sessions with Sam, I have a solid understand what needed to build an online store integrated with Sam's magic. Keeping kids happy is high stakes and I am not going to mess this up.
...
From Sam's perspective, his magic and "portal technology" are key differentiators and is "core to his business". The order management subdomain supports Sam.

From my perspective, the Order Mgmt subdomain is crucial. I want to ensure it scales to meet snackers' wishes amd the lifecycle of a customer order does not impact receiving "wish commands". So I treat it as a "core subdomain" - meaning custom software tailored to Sam's magic.
...
I document the design using the C4 modeling framework to provide a system context view, container view, and various component-level views I find useful. 
...
Adding "context to code" has helped my continuous learning journey allowing me to build many enhancements beyond a "do and forget" tutorial.

### Demo the Result

From the design I start coding and integrated within my local Dockerized Playground. During implementation, I also wanted to compare Kafka vs Redpanda - a competing, Kafka-compatible stream processing platform. I verify that the stream processing engine can be swapped out without impacting the snacks unlimited applications. 

BONUS: I recently was able to swap-in Warpstream another Kafka-compatible "engine" - acquired by Confluent. I pushed this to the playground GitHub repo.
...
That said, today is our lucky day. Before going "Live", Sam has decided to run a short-term promotion to test out the system and his magic. Submit your wish through this Snack Unlimited Promotion App. The first four customers will get a Snickers :-)
